...
function AccordionItem({ cp, expanded, saving, currentTime, pid, ks, onToggle, onSave, onDelete }: ItemProps) {
    ...
    >
      ...
      >
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, minWidth: 0, overflow: 'hidden', width: '100%' }}>
          <Typography sx={{ fontSize: 11, fontWeight: 700, color: D.muted, flexShrink: 0, fontVariantNumeric: 'tabular-nums', minWidth: 32 }}>
            {msToDisplay(cp.startTime)}
          </Typography>
...
export const RightPanel = ({
  ...
  const [tab, setTab] = useState<'chapter'|'slide'>(activeType);
  ...
  const submitNew = () => {
    ...
    onSave({ type: tab, startTime: displayToMs(newTimeStr), title: newTitle, description: newDescription, tags: '', imageFile: newImageFile });
  };
  ...
  const listItems = tab === 'chapter' ? chapters : slides;

  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', backgroundColor: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(16px)', overflow: 'hidden' }}>

      {/* ── Tabs ── */}
      <Box sx={{ display: 'flex', borderBottom: `1px solid ${D.border}`, flexShrink: 0 }}>
        {(['chapter','slide'] as const).map(t => (
          <Box key={t} onClick={() => setTab(t)} sx={{
            flex: 1, display: 'flex', justifyContent: 'center', py: 1.5, cursor: 'pointer',
            borderBottom: tab === t ? '2px solid #006efa' : '2px solid transparent',
            '&:hover': { backgroundColor: 'rgba(255,255,255,0.05)' },
          }}>
            {t === 'chapter' ? <ChapterTabIcon active={tab===t}/> : <SlideTabIcon active={tab===t}/>}
          </Box>
        ))}
      </Box>

      {/* ── New item form (appears at top when adding) ── */}
      {isNewForm && (
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
            {tab === 'slide' && (
              <Box
                ...
                {newImageFile
                  ? <Typography sx={{ fontSize: 11, color: '#006efa' }}>{newImageFile.name}</Typography>
            ...
            <TextField value={newTitle} onChange={e => e.target.value.length <= 75 && setNewTitle(e.target.value)}
              size="small" fullWidth placeholder={tab === 'slide' ? 'Slide name' : 'Chapter name'} sx={inputSx}
              autoFocus/>
            <Box sx={{ display: 'flex', gap: 1, alignItems: 'center' }}>
            ...
            <Box sx={{ display: 'flex', gap: 1 }}>
              <Button size="small" onClick={onClose} sx={{ flex: 1, color: D.muted, border: `1px solid ${D.border}`, textTransform: 'none', fontSize: 12, borderRadius: '4px' }}>
                Cancel
              </Button>
      ...
      <Box sx={{ flex: 1, overflowY: 'auto', minHeight: 0,
        ...
        {listItems.length === 0 && !isNewForm ? (
          /* Empty state */
          <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', p: 3, pt: 4, gap: 1.5, textAlign: 'center' }}>
            {/* Icon */}
            ...
            <Typography sx={{ fontSize: 14, fontWeight: 700, color: D.text }}>
              {tab === 'chapter' ? 'Chapters' : 'Slides'}
            </Typography>
            ...
            <Button
              size="small"
              startIcon={<span style={{ fontSize: 16, fontWeight: 700 }}>+</span>}
              onClick={() => onAddNew(tab)}
              sx={{ mt: 1, backgroundColor: 'transparent', color: '#006efa', border: '1px solid #006efa', textTransform: 'none', fontSize: 13, fontWeight: 700, borderRadius: '4px', px: 2, '&:hover': { backgroundColor: 'rgba(0,110,250,0.1)' } }}
            >
              Add {tab === 'chapter' ? 'chapter' : 'slides'}
            </Button>
          </Box>
        ) : listItems.map(cp => (
          <AccordionItem
            ...
            onToggle={() => {
              if (cp.id === editingId) {
                // collapse — keep panel open, just deselect
                onSelectCuePoint({ ...cp, id: '' } as any); // signal deselect
              } else {
                onSelectCuePoint(cp);
              }
            }}
